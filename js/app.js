    ...
    async function runSmartImport() {
        ...
        try {
            ...
            if (resultsEl) resultsEl.innerHTML = e.message === 'NO_KEY'
                ? '<p style="text-align:center;color:#8e8e93;">Add Gemini API key in Settings for AI Import!</p><p style="text-align:center;font-size:12px;color:#8e8e93;margin-top:8px;">Or paste one task per line and click manual import.</p><button onclick="manualImport()" style="background:var(--primary);color:white;border:none;border-radius:10px;padding:10px;width:100%;font-weight:700;cursor:pointer;margin-top:8px;">📝 Manual Line-by-Line Import</button>'
                : `<p style="text-align:center;color:#ff3b30;">Error: ${e.message}</p>`;
        }
    }
    ...
    function addSwipeToComplete(element, taskId) {
        ...
        element.addEventListener('touchend', e => {
            ...
            else if (diff < -80) { pinReminder(taskId); hapticFeedback('light'); }
        }, { passive: true });
    }
