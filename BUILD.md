...
This app has no build step today — `index.html` loads 54 plain `<script>`
...
Concatenates all 54 JS files (in the exact order `index.html` loads them)
into `dist/app.bundle.js`, syntax-checks the result, generates a matching
`dist/index.html` (same file, 54 `<script>` tags collapsed into 1), and
...
(54 requests → 1); minification is a separate, larger win worth doing with
a real tool.
