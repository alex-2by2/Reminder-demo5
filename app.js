...
// ============================================================
// PHASE 14 LOADER / STARTUP RESCUE FIX
// ============================================================
(function(){
    if (window.__phase14LoaderRescueApplied) return;
    window.__phase14LoaderRescueApplied = true;
    ...
    function safeHideSplash(reason) {
        try {
            const splash = getEl('splashScreen');
            if (!splash) {
                window.AppBoot.splashHidden = true;
                return;
            }
            splash.style.opacity = '0';
            setTimeout(() => {
                splash.style.display = 'none';
                window.AppBoot.splashHidden = true;
                console.log('[Phase14] Splash hidden:', reason || 'no-reason');
            }, 220);
        } catch (e) {
            console.error('safeHideSplash failed', e);
            window.AppBoot.bootErrors.push(String(e));
        }
    }

    function showBootFallback(message) {
        try {
            let box = getEl('bootFallbackBox');
            if (!box) {
                box = document.createElement('div');
                box.id = 'bootFallbackBox';
                box.className = 'boot-fallback-box';
                box.innerHTML = '<div class="boot-fallback-title">App recovered from startup issue</div><div id="bootFallbackMsg" class="boot-fallback-msg"></div>';
                document.body.appendChild(box);
            }
            const msg = getEl('bootFallbackMsg');
            if (msg) msg.textContent = message || 'Some startup modules failed, but the app should still open.';
            box.style.display = 'block';
        } catch (e) {
            console.error('showBootFallback failed', e);
        }
    }

    window.completeAppBoot = function(reason) {
        if (window.AppBoot.bootCompleted) return;
        window.AppBoot.bootCompleted = true;
        safeHideSplash(reason || 'boot-complete');
        try {
            if (typeof refreshAllPatchedModules === 'function') refreshAllPatchedModules();
        } catch(e) {
            console.error('completeAppBoot refresh failed', e);
        }
    };

    window.failSafeBoot = function(reason) {
        console.warn('[Phase14] failSafeBoot:', reason);
        showBootFallback('Startup recovered using fallback mode.');
        safeHideSplash(reason || 'failsafe');
    };

    // Global startup error catcher
    window.addEventListener('error', function(ev){
        try {
            const msg = ev && ev.message ? ev.message : 'Unknown startup error';
            window.AppBoot.bootErrors.push(msg);
            if (!window.AppBoot.splashHidden) {
                failSafeBoot('window-error');
            }
        } catch(_) {}
    });

    // Run a guarded boot sequence after DOM is ready
    function runPhase14BootSequence() {
        try {
            // Phase refreshes are allowed, but no single failure should block boot
            try { if (typeof runPhaseStartup === 'function') runPhaseStartup(); } catch(e) { console.error(e); }
            try { if (typeof refreshAfterAuthChange === 'function' && window.currentUser) refreshAfterAuthChange(); } catch(e) { console.error(e); }
            try { if (typeof refreshAllPatchedModules === 'function') refreshAllPatchedModules(); } catch(e) { console.error(e); }
            try { if (typeof checkDueRemindersSafe === 'function') checkDueRemindersSafe(); } catch(e) { console.error(e); }

            setTimeout(() => completeAppBoot('phase14-sequence'), 900);
        } catch (e) {
            console.error('runPhase14BootSequence failed', e);
            failSafeBoot('phase14-sequence-error');
        }
    }

    document.addEventListener('DOMContentLoaded', () => {
        setTimeout(runPhase14BootSequence, 120);
    });

    // Absolute fallback: never keep splash forever
    setTimeout(() => {
        if (!window.AppBoot.splashHidden) {
            failSafeBoot('absolute-timeout');
        }
    }, 4500);
})();
