...
// --- FIREBASE INITIALIZATION & OFFLINE PERSISTENCE ---
    ...
    document.addEventListener("DOMContentLoaded", () => {
        ...
        if(appPinCode && appLockToggleEl) appLockToggleEl.checked = true;
    ...
    // --- Morning Briefing ---
    let morningBriefingRetries = 0;
    function checkMorningBriefing() {
        // Wait briefly for auth, but do not retry forever
        if(!currentUser) {
            if (morningBriefingRetries >= 8) return;
            morningBriefingRetries++;
            setTimeout(checkMorningBriefing, 1000);
            return;
        }
        morningBriefingRetries = 0;
    ...
    function logoutUser() { 
        auth.signOut().then(() => { 
            ...
            const preserveKeys = [
                "appTheme","darkMode","autoDark","widgetPrefs","haptic",
                "appFontSize","eventColor","holidayColor","appPin","isPro"
            ];
            const preserved = {};
    ...
    function syncToCloud() {
        ...
        if (syncEl) syncEl.innerText = "☁️ Saving...";
        ...
        syncTimeout = setTimeout(() => {
            ...
            db.collection("users").doc(currentUser.uid).set(dataToSave, {merge: true}).then(() => { 
                const syncEl = document.getElementById("syncStatusText");
                if (syncEl) syncEl.innerText = "☁️ Synced"; 
            }).catch((e) => { 
                console.error("Sync error:", e);
                const syncEl = document.getElementById("syncStatusText");
                if (syncEl) syncEl.innerText = "⚠️ Sync Error"; 
            });
        }, 2000);
    }
    ...
    function openMatrixModal() {
        ...
        pending.forEach(r => {
            const isUrgent = r.time ? (r.time.split('T')[0] <= today) : false; 
    ...
    function renderProductivityHeatmap() {
        ...
        reminders.filter(r => r.status === 'completed' && r.time).forEach(r => {
            const d = r.time.split('T')[0];
    ...
    function checkPreAlarmNotifications() {
        ...
        const now = new Date().getTime();
        let changed = false;
        reminders.filter(r => r.status !== 'completed' && !r.archived && r.preAlarm > 0 && !r.preNotified && r.time).forEach(r => {
        ...
        if (changed) saveReminderArray(reminders, true);
    }
    ...
    function updateNextTaskWidget() {
        ...
        const upcoming = reminders
            .filter(r => r.status !== 'completed' && !r.archived && r.time && !isNaN(new Date(r.time).getTime()) && new Date(r.time) > now)
            .sort((a,b) => new Date(a.time) - new Date(b.time));
...
function saveReminderArray(reminders, doSync = true) {
    localStorage.setItem("reminders", JSON.stringify(Array.isArray(reminders) ? reminders : []));
...
function normalizeReminder(rem) {
    ...
    return {
        id: rem.id || Date.now(),
        task: (rem.task || '').toString().trim(),
        notes: rem.notes || '',
        time: rem.time || '',
        repeat: rem.repeat || 'none',
        priority: rem.priority || 'medium',
        status: rem.status || 'pending',
        archived: !!rem.archived,
        preAlarm: Number(rem.preAlarm || 0),
        preNotified: !!rem.preNotified,
        updatedAt: rem.updatedAt || Date.now()
    };
}

function validateReminderInput(rem) {
    if (!rem.task || !String(rem.task).trim()) {
        showToast("Task title required", "error");
        return false;
    }
    if (rem.time && isNaN(new Date(rem.time).getTime())) {
        showToast("Invalid task time", "error");
        return false;
    }
    return true;
}

// startup cleanup for corrupted reminders
document.addEventListener('DOMContentLoaded', () => {
    try {
        const cleaned = getReminderArray().map(normalizeReminder).filter(Boolean);
        saveReminderArray(cleaned, false);
    } catch (e) {
    ...
    window.safeMutateReminders = function(mutator, opts = {}) {
        try {
            ...
            if (typeof afterAnyDataMutation === 'function') afterAnyDataMutation();
                    if (typeof refreshAfterAuthChange === 'function') refreshAfterAuthChange();
            return true;
        } catch (e) {
...
// ============================================================
// PHASE 9 AUTH + SYNC HARDENING
// ============================================================

(function(){
    if (window.__phase9AuthSyncApplied) return;
    window.__phase9AuthSyncApplied = true;

    // ---- user-specific storage key helper ----
    window.getUserScopedKey = function(baseKey) {
        try {
            const uid = (window.currentUser && window.currentUser.uid) ? window.currentUser.uid : 'guest';
    ...
    // ---- safer cloud sync wrapper with basic in-flight guard ----
    window.__syncInFlight = false;
    if (typeof window.syncToCloud === 'function' && !window.__phase9SyncWrapped) {
        const originalSyncToCloud = window.syncToCloud;
        window.syncToCloud = async function(...args) {
            if (!window.currentUser) return;
            if (!navigator.onLine) return;
            if (window.__syncInFlight) return;
            window.__syncInFlight = true;
            try {
                const result = await originalSyncToCloud.apply(this, args);
                return result;
            } catch (e) {
                console.error('Phase9 syncToCloud wrapper failed', e);
            } finally {
                setTimeout(() => { window.__syncInFlight = false; }, 500);
            }
        };
        window.__phase9SyncWrapped = true;
    }

    // ---- safer logout cleanup helper ----
    window.cleanupUserScopedCache = function() {
        try {
            const preserve = ['appTheme','darkMode','autoDark','widgetPrefs','haptic','appFontSize','eventColor','holidayColor','appPin','isPro'];
            const preserved = {};
            preserve.forEach(k => {
                const v = localStorage.getItem(k);
                if (v !== null) preserved[k] = v;
            ...
            console.error('cleanupUserScopedCache failed', e);
        }
    };

    // ---- auth-state refresh helper ----
    window.refreshAfterAuthChange = function() {
        try { if (typeof refreshReminderUI === 'function') refreshReminderUI(); } catch(e){}
        try { if (typeof refreshPhase5Modules === 'function') refreshPhase5Modules(); } catch(e){}
        try { if (typeof refreshPhase6Modules === 'function') refreshPhase6Modules(); } catch(e){}
        try { if (typeof updateNextTaskWidget === 'function') updateNextTaskWidget(); } catch(e){}
    };

    // ---- patch logoutUser to use cleanup helper if function exists ----
    if (typeof window.logoutUser === 'function' && !window.__phase9LogoutWrapped) {
        const originalLogoutUser = window.logoutUser;
        window.logoutUser = function(...args) {
            try {
                cleanupUserScopedCache();
                return originalLogoutUser.apply(this, args);
            } catch (e) {
                console.error('logoutUser wrapper failed', e);
                try {
        ...
        window.__phase9LogoutWrapped = true;
    }

    // ---- startup auth-sync sanity check ----
    document.addEventListener('DOMContentLoaded', () => {
        setTimeout(() => {
            try {
                if (window.currentUser) refreshAfterAuthChange();
            } catch(e) {
                console.error('Phase9 auth startup refresh failed', e);
            }
        }, 1200);
    });
})();
...
// ============================================================
// PHASE 10 FAMILY SHARING + PROJECT LINKING DEEP FIX
// ============================================================

(function(){
    if (window.__phase10FamilyProjectApplied) return;
    window.__phase10FamilyProjectApplied = true;

    // ---------- project linking helpers ----------
    window.getProjectByIdSafe = function(projectId) {
        const projects = safeStorage('projects', []);
        if (!Array.isArray(projects)) return null;
        return projects.find(p => String(p.id) === String(projectId)) || null;
    };

    window.ensureReminderProjectLink = function(reminder) {
        if (!reminder) return reminder;
        if (!reminder.projectId) return reminder;
        const p = getProjectByIdSafe(reminder.projectId);
        if (p) {
            reminder.projectName = p.name || reminder.projectName || '';
            reminder.projectEmoji = p.emoji || reminder.projectEmoji || '';
            reminder.projectColor = p.color || reminder.projectColor || '';
        }
        return reminder;
    };

    // patch reminder normalization path if available
    if (typeof window.normalizeReminder === 'function' && !window.__phase10NormalizeWrapped) {
        const originalNormalizeReminder = window.normalizeReminder;
        window.normalizeReminder = function(rem) {
            const out = originalNormalizeReminder(rem);
            return ensureReminderProjectLink(out);
        };
        window.__phase10NormalizeWrapped = true;
    }

    // ---------- family shared task safer rendering ----------
    window.getSharedTasksRenderable = function() {
        const tasks = safeStorage('sharedTasks', []);
        if (!Array.isArray(tasks)) return [];
        return tasks.map(t => ({
            id: t.id || Date.now(),
            title: t.title || t.task || 'Shared Task',
            from: t.from || 'Unknown',
    ...
    window.renderSharedTasksSafe = function() {
        try {
            if (typeof renderSharedTasks === 'function') {
                return renderSharedTasks();
            }
        } catch (e) {
            console.error('renderSharedTasks original failed, fallback used', e);
        }

        const container = document.getElementById('sharedTasksContainer');
        if (!container) return;

        const tasks = getSharedTasksRenderable();
        if (!tasks.length) {
            container.innerHTML = '<div class="shared-task-empty">No shared tasks yet.</div>';
            return;
        }

        container.innerHTML = tasks.map(t => `
            <div class="shared-task-card">
                <div class="shared-task-title">${sanitizeHTML ? sanitizeHTML(t.title) : t.title}</div>
                <div class="shared-task-meta">From: ${sanitizeHTML ? sanitizeHTML(t.from) : t.from}</div>
                <div class="shared-task-meta">Status: ${sanitizeHTML ? sanitizeHTML(t.status) : t.status}</div>
            </div>
        `).join('');
    };

    // ---------- share task wrapper with task metadata ----------
    if (typeof window.shareTaskWithFamily === 'function' && !window.__phase10ShareWrapped) {
        const originalShareTask = window.shareTaskWithFamily;
        window.shareTaskWithFamily = async function(taskId, ...args) {
            try {
                return await originalShareTask.call(this, taskId, ...args);
            } catch (e) {
                console.error('Phase10 shareTaskWithFamily fallback', e);
                const reminders = safeStorage('reminders', []);
                const task = Array.isArray(reminders)
                    ? reminders.find(r => String(r.id) === String(taskId))
                    : null;

                addSharedTaskLocalFallback({
                    id: Date.now(),
                    title: task?.task || 'Pending shared task',
                    from: 'You',
                    status: 'pending'
                });
                renderSharedTasksSafe();
                showToast('Shared locally only (cloud share failed)', 'warning');
            }
        };
        window.__phase10ShareWrapped = true;
    }

    // ---------- refresh hook ----------
    window.refreshPhase10Modules = function() {
        try { renderSharedTasksSafe(); } catch(e){}
    };
})();


// ============================================================
// PHASE 11 DASHBOARD + UI POLISH + SETTINGS CLEANUP
// ============================================================

(function(){
    if (window.__phase11UIPolishApplied) return;
    window.__phase11UIPolishApplied = true;

    // ---------- dashboard safe refresh ----------
    window.refreshDashboardSafe = function() {
        try { if (typeof updateNextTaskWidget === 'function') updateNextTaskWidget(); } catch(e){}
        try { if (typeof renderFinanceSummarySafe === 'function') renderFinanceSummarySafe(); } catch(e){}
        try { if (typeof renderMoodTracker === 'function') renderMoodTracker(); } catch(e){}
        try { if (typeof renderHabits === 'function') renderHabits(); } catch(e){}
        try { if (typeof refreshWidgetCardsSafe === 'function') refreshWidgetCardsSafe(); } catch(e){}
        try { if (typeof renderSharedTasksSafe === 'function') renderSharedTasksSafe(); } catch(e){}
    };
    ...
    window.openModalSafe = function(modalId) {
        try {
            ...
            console.error('openModalSafe failed', modalId, e);
        }
    };
    ...
    window.closeModalSafe = function(modalId) {
        try {
            ...
            console.error('closeModalSafe failed', modalId, e);
        }
    };

    // ---------- settings fallback refresh ----------
    window.refreshSettingsUI = function() {
        try {
            const appLockToggle = document.getElementById('appLockToggle');
            if (appLockToggle && localStorage.getItem('appPin')) {
                appLockToggle.checked = true;
            }
        } catch (e) {
            console.error('refreshSettingsUI failed', e);
        }
    };

    // ---------- empty-state normalizer ----------
    window.ensureEmptyState = function(containerId, html) {
        const el = document.getElementById(containerId);
        if (!el) return;
        const hasRealChildren = el.children && el.children.length > 0;
        const hasText = (el.textContent || '').trim().length > 0;
        if (!hasRealChildren && !hasText) {
            el.innerHTML = html;
        }
    };

    // ---------- phase 11 refresh ----------
    window.refreshPhase11Modules = function() {
        try { refreshDashboardSafe(); } catch(e){}
        try { refreshSettingsUI(); } catch(e){}
        try { ensureEmptyState('sharedTasksContainer', '<div class="shared-task-empty">No shared tasks yet.</div>'); } catch(e){}
        try { ensureEmptyState('projectsList', '<div class="project-empty-state">No projects yet.</div>'); } catch(e){}
        try { ensureEmptyState('habitsList', '<div class="habit-empty-state">No habits yet.</div>'); } catch(e){}
    };

    document.addEventListener('DOMContentLoaded', () => {
        setTimeout(() => {
            try { refreshPhase11Modules(); } catch(e){}
        }, 1400);
    });
})();
...
// ============================================================
// PHASE 12 NOTIFICATIONS + SERVICE WORKER + BACKGROUND REMINDER HARDENING
// ============================================================

(function(){
    if (window.__phase12NotifApplied) return;
    window.__phase12NotifApplied = true;

    // ---------- notification permission helper ----------
    window.ensureNotificationPermissionSafe = async function() {
        try {
            ...
            console.error('ensureNotificationPermissionSafe failed', e);
            return false;
        }
    };

    // ---------- unified reminder notifier ----------
    window.fireReminderNotificationSafe = async function(title, body) {
        try {
            const ok = await ensureNotificationPermissionSafe();
            if (!ok) return false;

            if (navigator.serviceWorker && navigator.serviceWorker.ready) {
                const reg = await navigator.serviceWorker.ready;
                await reg.showNotification(title || 'Reminder', {
                    body: body || '',
                    icon: '/icon-192.png',
                    badge: '/icon-192.png',
                    tag: 'master-reminder'
            ...
            console.error('fireReminderNotificationSafe failed', e);
            return false;
        }
    };

    // ---------- due reminder checker ----------
    window.checkDueRemindersSafe = function() {
        try {
            const reminders = safeStorage('reminders', []);
            ...
            reminders.forEach(r => {
                if (!r || r.status === 'completed' || r.archived || !r.time) return;
                const due = new Date(r.time).getTime();
                if (isNaN(due)) return;

                if (now >= due && !r.dueNotified) {
                    fireReminderNotificationSafe(r.task || 'Reminder', 'Task is due now');
                    r.dueNotified = true;
                    r.updatedAt = Date.now();
                    changed = true;
                }
            ...
            console.error('checkDueRemindersSafe failed', e);
        }
    };

    // ---------- periodic in-page reminder scan ----------
    if (!window.__phase12ReminderInterval) {
        window.__phase12ReminderInterval = setInterval(() => {
            try {
                checkPreAlarmNotifications && checkPreAlarmNotifications();
            } catch(e) {}
            try {
                checkDueRemindersSafe();
            } catch(e) {}
        }, 60000);
    }

    // ---------- startup reminder check ----------
    document.addEventListener('DOMContentLoaded', () => {
        setTimeout(() => {
            try { checkDueRemindersSafe(); } catch(e){}
        }, 2000);
    });
})();
...
// ============================================================
// PHASE 13A CALENDAR + RECURRING + ICS EXPORT CLEANUP
// ============================================================

(function(){
    if (window.__phase13CalendarApplied) return;
    window.__phase13CalendarApplied = true;

    // ---------- repeat helpers ----------
    window.computeNextReminderTime = function(reminder) {
        if (!reminder || !reminder.time || !reminder.repeat || reminder.repeat === 'none') return null;
        const d = new Date(reminder.time);
        if (isNaN(d.getTime())) return null;

        const next = new Date(d);
        switch (String(reminder.repeat).toLowerCase()) {
            case 'daily':
    ...
    window.resetReminderNotificationFlags = function(reminder) {
        if (!reminder) return reminder;
        reminder.preNotified = false;
        reminder.dueNotified = false;
        reminder.updatedAt = Date.now();
        return reminder;
    };

    // ---------- recurring reminder rollover ----------
    window.rolloverRecurringReminderIfNeeded = function(reminder) {
        if (!reminder || reminder.status === 'completed' || reminder.archived) return reminder;
        if (!reminder.time || !reminder.repeat || reminder.repeat === 'none') return reminder;

        const due = new Date(reminder.time).getTime();
        if (isNaN(due)) return reminder;

        const now = Date.now();
        if (now < due) return reminder;

        const nextTime = computeNextReminderTime(reminder);
        if (!nextTime) return reminder;

        reminder.time = nextTime;
        reminder = resetReminderNotificationFlags(reminder);
        return reminder;
    };

    // ---------- calendar grouping helper ----------
    window.groupRemindersByDateSafe = function(reminders) {
        const out = {};
    ...
    window.runRecurringMaintenance = function() {
        try {
            const reminders = safeStorage('reminders', []);
            if (!Array.isArray(reminders)) return;
            ...
            const updated = reminders.map(r => {
                ...
                const out = rolloverRecurringReminderIfNeeded(r);
            ...
            console.error('runRecurringMaintenance failed', e);
        }
    };

    // ---------- safer ICS export wrapper ----------
    if (typeof window.exportToICS === 'function' && !window.__phase13ICSWrapped) {
        const originalExportToICS = window.exportToICS;
        window.exportToICS = function(...args) {
            try {
                runRecurringMaintenance();
                return originalExportToICS.apply(this, args);
            } catch (e) {
                console.error('exportToICS wrapper failed', e);
                showToast('ICS export failed', 'error');
            }
        };
        window.__phase13ICSWrapped = true;
    }

    // ---------- startup + interval ----------
    document.addEventListener('DOMContentLoaded', () => {
        setTimeout(() => {
            try { runRecurringMaintenance(); } catch(e){}
        }, 2500);
    });

    if (!window.__phase13RecurringInterval) {
        window.__phase13RecurringInterval = setInterval(() => {
            try { runRecurringMaintenance(); } catch(e){}
        }, 5 * 60 * 1000);
    }
})();
...
// ============================================================
// PHASE 13B DEAD-CODE CLEANUP / CODE PRUNING PASS
// ============================================================

(function(){
    if (window.__phase13BPruneApplied) return;
    window.__phase13BPruneApplied = true;

    // Consolidated no-op cleanup registry for old patch-era helpers
    window.LegacyCleanup = window.LegacyCleanup || {
        disabledIntervals: [],
        disabledWrappers: [],
        notes: []
    };

    // Central function existence checker
    window.fnExists = function(name) {
        return typeof window[name] === 'function';
    };

    // Safe one-time wrapper installer to reduce patch-on-patch noise
    window.installWrapperOnce = function(fnName, key, installer) {
        try {
            window.__wrapperInstallRegistry = window.__wrapperInstallRegistry || {};
            if (window.__wrapperInstallRegistry[key]) return false;
            if (typeof window[fnName] !== 'function') return false;
            const original = window[fnName];
            const wrapped = installer(original);
            if (typeof wrapped !== 'function') return false;
            window[fnName] = wrapped;
            window.__wrapperInstallRegistry[key] = true;
            return true;
        } catch (e) {
            console.error('installWrapperOnce failed for', fnName, e);
            return false;
        }
    };

    // Normalize old refresh calls into a single pipeline
    window.refreshAllPatchedModules = function() {
        try { if (fnExists('refreshReminderUI')) refreshReminderUI(); } catch(e){}
        try { if (fnExists('refreshPhase5Modules')) refreshPhase5Modules(); } catch(e){}
        try { if (fnExists('refreshPhase6Modules')) refreshPhase6Modules(); } catch(e){}
        try { if (fnExists('refreshPhase10Modules')) refreshPhase10Modules(); } catch(e){}
        try { if (fnExists('refreshPhase11Modules')) refreshPhase11Modules(); } catch(e){}
    };

    // Patch afterAnyDataMutation once so all old paths converge here
    if (typeof window.afterAnyDataMutation === 'function' && !window.__phase13BAfterMutationWrapped) {
        const originalAfterAnyDataMutation = window.afterAnyDataMutation;
        window.afterAnyDataMutation = function(...args) {
            try {
                originalAfterAnyDataMutation.apply(this, args);
            } catch(e) {
                console.error('afterAnyDataMutation original failed', e);
            }
            try { refreshAllPatchedModules(); } catch(e){}
        };
        window.__phase13BAfterMutationWrapped = true;
    }

    // Safe startup convergence
    if (!window.__phase13BStartupBound) {
        window.__phase13BStartupBound = true;
        document.addEventListener('DOMContentLoaded', () => {
            setTimeout(() => {
                try { refreshAllPatchedModules(); } catch(e){}
            }, 1800);
