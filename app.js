// Master Reminder App - JavaScript
...
// --- FIREBASE INITIALIZATION & OFFLINE PERSISTENCE ---
    ...
    document.addEventListener("DOMContentLoaded", () => {
        ...
        setTimeout(() => {
            const log = JSON.parse(localStorage.getItem('notifLog') || '[]');
    ...
    function syncToCloud() {
        ...
        syncTimeout = setTimeout(() => {
            const dataToSave = {
                ...
                vehicleLogs: JSON.parse(localStorage.getItem("vehicleLogs") || "[]"),
                birthdays: JSON.parse(localStorage.getItem("birthdays") || "[]"),
                homeManagement: JSON.parse(localStorage.getItem("homeManagement") || "[]"),
                quickNotes: JSON.parse(localStorage.getItem("quickNotes") || "[]"),
                pomodoroHistory: JSON.parse(localStorage.getItem("pomodoroHistory") || "[]").slice(0,50),
    ...
    function addExpense(type){const iE=type==='expense';const name=document.getElementById(iE?'expNameInput':'incNameInput').value.trim();const amt=Number(document.getElementById(iE?'expAmtInput':'incAmtInput').value);const cat=document.getElementById(iE?'expCatInput':'incCatInput').value;const date=document.getElementById(iE?'expDateInput':'incDateInput').value||getTodayStr();const note=iE?(document.getElementById('expNoteInput')?.value.trim()||''):'';if(!name||!amt)return showToast('Enter name & amount!','error');const d=getFinData();d[iE?'expenses':'income'].unshift({id:Date.now(),name,amount:amt,category:cat,date,type,note});saveFinData(d);document.getElementById(iE?'expNameInput':'incNameInput').value='';document.getElementById(iE?'expAmtInput':'incAmtInput').value='';if(iE&&document.getElementById('expNoteInput'))document.getElementById('expNoteInput').value='';renderFinanceDashboard();if(!iE)renderIncome();hapticFeedback('success');showToast(iE?'Expense added!':'Income added!','success')}
    ...
    function renderAppStats() {
        ...
        const totalMins = JSON.parse(localStorage.getItem('pomodoroHistory')||'[]').reduce((s,h)=>s+h.mins,0);
        ...
        if(details) details.innerHTML = `
            ...
            💊 Medicines tracked: ${JSON.parse(localStorage.getItem('medicines')||'[]').length}<br>
            🎂 Birthdays tracked: ${JSON.parse(localStorage.getItem('birthdays')||'[]').length}<br>
            💰 Total expenses recorded: ${finData.expenses.length}<br>
            ✈️ Trips planned: ${JSON.parse(localStorage.getItem('travelData')||'{"trips":[]}').trips.length}`;
    ...
    function renderAutomationQuickRules() {
        ...
        const enabled = JSON.parse(localStorage.getItem('enabledRules') || '["r1","r2","r3","r4","r5"]');
    ...
    function toggleQuickRule(id, on) {
        let enabled = JSON.parse(localStorage.getItem('enabledRules') || '["r1","r2","r3","r4","r5"]');
    ...
    function deleteCustomRule(id) {
        const rules = JSON.parse(localStorage.getItem('customRules') || '[]').filter(r => r.id !== id);
        localStorage.setItem('customRules', JSON.stringify(rules));
        renderCustomRulesList();
    }
    ...
    // NOTIFICATION CENTRE
    // ============================================================
    function getNotifLog() { return JSON.parse(localStorage.getItem('notifLog') || '[]'); }
    ...
    // SAVINGS GOAL
    // ============================================================
    function getSavingsGoals() { return JSON.parse(localStorage.getItem('savingsGoals') || '[]'); }
    ...
    function renderStudyAnalytics() {
        ...
        const pomoHist = JSON.parse(localStorage.getItem('pomodoroHistory') || '[]');
    ...
    function addDependency() {
        ...
        let deps = JSON.parse(localStorage.getItem('taskDeps') || '[]');
    ...
    function renderDependenciesList() {
        ...
        const deps = JSON.parse(localStorage.getItem('taskDeps') || '[]');
        const reminders = JSON.parse(localStorage.getItem('reminders') || '[]');
    ...
    function removeDependency(id) {
        const deps = JSON.parse(localStorage.getItem('taskDeps') || '[]').filter(d => d.id !== id);
        localStorage.setItem('taskDeps', JSON.stringify(deps));
    ...
    function renderHealthChecklist() {
        ...
        const saved = JSON.parse(localStorage.getItem('healthCheck_'+getTodayStr()) || '[]');
    ...
    // RECURRING EXPENSES
    // ============================================================
    function getRecurringExps() { return JSON.parse(localStorage.getItem('recurringExps') || '[]'); }
    ...
    const _origRenderFinDash = renderFinanceDashboard;
    renderFinanceDashboard = function() {
        _origRenderFinDash();
        const summary = renderMonthlyFinanceSummary();
        const el = document.getElementById('finMonthSummary');
        if(el) {
            el.innerHTML = 'Savings rate: <b style="color:'+( summary.savingRate>=20?'#34c759':'#ff9500')+'">'+summary.savingRate+'%</b> this month';
        }
    };
    ...
    // WARRANTY TRACKER
    // ============================================================
    function getWarranties() { return JSON.parse(localStorage.getItem('warranties') || '[]'); }
    ...
    // ============================================================
    function getVehicleLogs() { return JSON.parse(localStorage.getItem('vehicleLogs') || '[]'); }
